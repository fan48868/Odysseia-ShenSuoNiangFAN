...
def _check_power_limits(base: Any, exponent: Any) -> None:
    """拦截可能导致卡死的幂运算规模。"""
    if isinstance(exponent, (int, float)) and abs(exponent) > _MAX_POWER_EXPONENT:
        log.warning("幂运算指数过大，触发熔断: exponent=%s", exponent)
        raise asyncio.TimeoutError("指数过大")

    if isinstance(base, int) and isinstance(exponent, int):
        base_bits = abs(base).bit_length()
        estimated_bits = base_bits * abs(exponent)
        if estimated_bits > _MAX_RESULT_BITS:
            log.warning(
                "幂运算结果位数过大，触发熔断: base_bits=%s, exponent=%s, estimated_bits=%s",
                base_bits,
                exponent,
                estimated_bits,
            )
            raise asyncio.TimeoutError("结果过大")
...
def _safe_eval(node: ast.AST) -> float:
    ...
    if isinstance(node, ast.Constant):  # Python 3.8+ 支持的常量节点
        ...
        if isinstance(node.op, ast.Pow):
            _check_power_limits(left, right)
    ...
    **kwargs,
    ...
    try:
        ...
        calc_result = await asyncio.wait_for(
            asyncio.to_thread(_calculate, clean_expr),
            timeout=5.0,
        )
        ...
        log.warning(f"计算超时触发熔断: {expression}")
