...
async def analyze_image_with_gemini_pro(
    ...
    **kwargs,
    ...
    for item in image_context_list[:max_images]:
        ...
        try:
            image_bytes, final_mime_type = await asyncio.to_thread(
                sanitize_image, image_bytes
            )
        except Exception as e:
            log.warning(
                "图片压缩失败，将回退使用原图继续深度识图。"
                f" index={image_index}, source={source}, error={e}"
            )
    ...
    try:
        ...
        result_data["error"] = "图像识别超时，已熔断"
        log.warning("analyze_image_with_gemini_pro 执行超时，已触发 40s 熔断。")
    except Exception as e:
